...
└── lib/
    ...
    ├── ui-shell.js        ← Tab 切换 + sessionStorage 状态保持 + 分卷大小选择器
    ├── task-manager.js    ← 任务列表渲染 + Job 生命周期管理 + 任务历史
    ├── encode-tab.js      ← 编码 Tab：文件选择、拖放排序、分卷大小、提交任务
    ├── decode-tab.js      ← 解码 Tab：多 BMP 选择、F2P6 分卷解码、单/批量下载
    ...
    └── coders/
        ...
        ├── f2p6-decode.js ← F2P6 解码器（extractFileData/DataRange、流式流式提取）
